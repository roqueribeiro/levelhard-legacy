...
function curso_info()
{
	...
	$QueryCursosApply = mysqli_query($bd_connect, $QueryCursos);
	$QueryCursosResults = mysqli_num_rows($QueryCursosApply); 
	if ($QueryCursosResults > 0)
	{
		while ($ResultCursosRow = mysqli_fetch_array($QueryCursosApply)) 
		{
...
function s_curso()
{
	...
	if($c_check)
	{
		...
		$QueryCheckedApply = mysqli_query($bd_connect, $QueryChecked);
		$QueryCheckedResults = mysqli_num_rows($QueryCheckedApply); 
		if ($QueryCheckedResults > 0)
		{
			while ($ResultCheckedRow = mysqli_fetch_array($QueryCheckedApply)) 
			{
