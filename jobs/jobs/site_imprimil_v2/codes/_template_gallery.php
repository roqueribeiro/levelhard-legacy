<?php
	...
	$url_path 		= $_POST["varActivePath"];
	$dir_relative 	= $site_path . $url_path;
	...
	function getGalleryImageFiles() {
		...
		global $json_return, $url_path, $dir_relative, $dir_absolute;
				...
				while (false !== ($entry = readdir($handle))) {
					if (is_file($dir_absolute . $entry)) {
						...
						if ($path_parts["extension"] != "thumb") {
							...
							//Retorno em JSON
							array_push($json_return, array(
								"thumb" => $url_path . $path_parts_renamed["filename"] . ".thumb",
								"image" => $url_path . $utf8_filename,
