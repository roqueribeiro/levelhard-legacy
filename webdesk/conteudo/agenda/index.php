...
$VAR_PES_GRU	= $_GET["pes_grupo"];
$VAR_PES_UF		= $_GET["pes_estado"];
...
switch ($VAR_PAG) {
	...
	case "logout":
		session_destroy();
		header('Location: ' . $_SERVER["PHP_SELF"] . '?pag=pagLista');
		break;
	case "loginMenu":
		if ($FORM_USRCOD == 1) print PagLoginMenu($NotLogin = 1);
		else print PagLoginMenu();
		break;
	case "pagMenu":
		print PagMenu();
		break;
	case "pagLista":
		if ($_SESSION['USR_ACE'] > 0) print PagLista();
		else print PagNotLogin('');
		break;
	case "pagNovoUsuario":
		if ($_SESSION['USR_ACE'] > 1) print PagNovoUsuario();
		else print PagNotLogin('');
		break;
	case "pagNovoGrupo":
		if ($_SESSION['USR_ACE'] > 0) print PagNovoGrupo();
		else print PagNotLogin('');
		break;
	case "pagNovoContato":
		if ($_SESSION['USR_ACE'] > 0) print PagNovoContato();
		else print PagNotLogin('');
		break;
	case "actUsuario":
		ActUsuario();
	...
	case "actContato":
		...
		break;
	case "delContato":
		mysqli_query($bd_conexao, "DELETE FROM TB_GND_CTO WHERE GND_CTO_COD = " . $FORM_CONCOD . "");
		header('Location: ' . $_SERVER["PHP_SELF"] . '?pag=pagLista');
		break;
	case "delUsuario":
		mysqli_query($bd_conexao, "DELETE FROM TB_WDK_USR WHERE WDK_USR_COD = " . $FORM_USRCOD . "");
		header('Location: ' . $_SERVER["PHP_SELF"] . '?pag=pagNovoUsuario');
		break;
	default:
		session_start();
		$html = "
			...
			$(document).ready(function(){
				...
				$('#theme_conteudo_itens').load('" . $_SERVER["PHP_SELF"] . "?pag=pagLista', function(){
...
function ActUsuario()
{
	...
	$BDINSERT[2] = $_GET["form_login"];
	$BDINSERT[3] = $_GET["form_senha"];
	$BDINSERT[4] = $_GET["form_acesso"];
	$BDINSERT[5] = $_GET["form_uscont"];
	$BDINSERT[6] = $_GET["form_usedit"];
	$BDINSERT[7] = $_GET["form_uscod"];
	...
	if ($BDINSERT[7]) {
		...
		UPDATE 
			TB_WDK_USR 
		SET 
			WDK_USR_NOM = '" . $BDINSERT[1] . "',
			WDK_USR_USR = '" . $BDINSERT[2] . "',
			WDK_USR_SNH = '" . $BDINSERT[3] . "',
			WDK_USR_ACE = '" . $BDINSERT[4] . "',
			WDK_USR_AUS = '" . $USCONT . "'
		WHERE 
			WDK_USR_COD = " . $BDINSERT[7] . ";
		";
		mysqli_query($bd_conexao, $Query);
		header('Location: ' . $_SERVER["PHP_SELF"] . '?pag=pagNovoUsuario&cod=' . $BDINSERT[9]);
	} else {
		$Query = "
		INSERT INTO
		TB_WDK_USR 
...
function ActGrupo()
{
	...
	";
	mysqli_query($bd_conexao, $Query);
	header('Location: ' . $_SERVER["PHP_SELF"] . '?pag=pagNovoGrupo');
}

function ActContato()
{
	global $bd_conexao, $FORM_CONCOD;
	...
	if ($FORM_CONCOD) {
		...
		header('Location: ' . $_SERVER["PHP_SELF"] . '?pag=pagNovoContato&concod=' . $FORM_CONCOD);
	} else {
		$Query = "
		INSERT INTO 
		TB_GND_CTO 
		...
		";
		$QueryApply = mysqli_query($bd_conexao, $Query);
		header('Location: ' . $_SERVER["PHP_SELF"] . '?pag=pagNovoContato');
	}
}
// ================ Corta Frases
...
function PagNotLogin($NotLogin)
{
	...
	$MenuLogin = "<script>$('#theme_bar_login').load('" . $_SERVER["PHP_SELF"] . "?pag=loginMenu&cod=0');</script>";

	$jquery = "
	<script type=\"text/javascript\">
	$(document).ready(function() {
		$(this).oneTime(2000, function() {				   
			AjaxLoad('theme_conteudo_itens','" . $_SERVER["PHP_SELF"] . "?pag=pagLista');
		});
	});
	</script>							
	";
	...
	switch ($NotLogin) {
		...
		case "LE02":
			...
			break;
		case "LE03":
			$html = $jquery . '<div id="NotLogin">Você já está logado como ' . $_SESSION['USR_USR'] . '!</div>';
			break;
		case "ALERT01":
			$html = "
			<div id=\"alertas\">
			...
			onclick=\"AjaxLoad('theme_conteudo_itens','" . $_SERVER["PHP_SELF"] . "?pag=delContato&concod=" . $FORM_CONCOD . "');$.fancybox.close();\">
...
function PagLogin()
{
	global $bd_conexao, $VAR_NOME, $VAR_SENHA;

	$html = "";
	$Query = "
	SELECT 
		WDK_USR_COD, WDK_USR_NOM, WDK_USR_USR, WDK_USR_SNH, WDK_USR_ACE, WDK_USR_AUS
	FROM 
		TB_WDK_USR
	WHERE
		WDK_USR_USR = '" . $VAR_NOME . "'
	";
	...
	if ($QueryResults > 0) {
		...
		while ($ResultRow = mysqli_fetch_array($QueryApply)) {
			$BDSHOW[1]	= $ResultRow["WDK_USR_COD"];
			$BDSHOW[2] 	= $ResultRow["WDK_USR_NOM"];
			$BDSHOW[3] 	= $ResultRow["WDK_USR_USR"];
			$BDSHOW[4] 	= $ResultRow["WDK_USR_SNH"];
			$BDSHOW[5] 	= $ResultRow["WDK_USR_ACE"];
			$BDSHOW[6] 	= $ResultRow["WDK_USR_AUS"];
		}

		if (!$_SESSION['USR_COD']) {
			if ($BDSHOW[4] == MD5($VAR_SENHA)) {
				if ($BDSHOW[6]) {
					$USR_AUS = $BDSHOW[1] . "," . $BDSHOW[6];
				} else {
					$USR_AUS = $BDSHOW[1];
				}
				...
				$_SESSION['USR_USR'] = $BDSHOW[3];
				$_SESSION['USR_ACE'] = $BDSHOW[5];
...
function PagCab()
{
	...
	<link rel=\"stylesheet\" type=\"text/css\" href=\"styles/html_style.css\"/>
		...
		<div id="theme_con" class="center">
			<div id="theme_con1">
				...
				<div id="theme_rodape">
					...
					<form id="PesqForm" action="' . $_SERVER["PHP_SELF"] . '" method="GET">
						<input type="hidden" name="pag" value="pagLista">
						<select name="pes_estado">
							<option value="">Selecione</option>
							' . CarreraListaUF('', 1) . '
						</select>
						<select name="pes_grupo">
							<option value="">Selecione</option>
							' . CarregaListaGrupo() . '
						</select>
...
function CarreraListaUF($BDEDIT, $search)
{
	global $bd_conexao;
	$html = "";
	...
	$QueryUFApply = mysqli_query($bd_conexao, $QueryUF);
	$QueryUFResults = mysqli_num_rows($QueryUFApply);

	if ($QueryUFResults > 0) {
		while ($ResultRow = mysqli_fetch_array($QueryUFApply)) {
			$BDUF[1]	= $ResultRow["GND_UF_COD"];
			$BDUF[2]	= $ResultRow["GND_UF_SIG"];
			$BDUF[3] 	= $ResultRow["GND_UF_NOM"];

			if ($BDUF[2] == $BDEDIT[11]) {
				$html .= "<option value=" . $BDUF[2] . " selected=\"selected\">" . $BDUF[3] . "</option>";
			} else {
				$html .= "<option value=" . $BDUF[2] . ">" . $BDUF[3] . "</option>";
			}
		}
	}
...
function CarregaListaGrupo()
{
	global $bd_conexao;

	$html = "";
	$QueryEdit = "SELECT * FROM TB_GND_GRU ORDER BY GND_GRU_TIT";

	$QueryEditApply = mysqli_query($bd_conexao, $QueryEdit);
	$QueryEditResults = mysqli_num_rows($QueryEditApply);

	if ($QueryEditResults > 0) {
		while ($ResultRow = mysqli_fetch_array($QueryEditApply)) {
			$result[1]	= $ResultRow["GND_GRU_COD"];
			$result[2] 	= $ResultRow["GND_GRU_TIT"];

			$html .= '<option value="' . $result[1] . '">' . $result[2] . '</option>';
		}
	}
...
function PagLoginMenu($NotLogin = 0)
{
		...
		$(document).ready(function(){
			$('#LoginForm').ajaxForm({ 
				...
				success: function() { 
					...
					$('#logo').fadeIn(2000);
					$('#theme_conteudo_itens').fadeIn('slow');
					$('#theme_menu').load('" . $_SERVER["PHP_SELF"] . "?pag=pagMenu');
				} 
			}); 
		});
		</script>
	...
	switch ($NotLogin) {
		case 0:
			$html = '
				<form id="LoginForm" action="' . $_SERVER["PHP_SELF"] . '" method="GET">
		...
		case 1:
			$html = '
				...
				onclick="AjaxLoad(\'theme_conteudo_itens\',\'' . $_SERVER["PHP_SELF"] . '?pag=logout\');$(\'#theme_menu\').load(\'' . $_SERVER["PHP_SELF"] . '?pag=pagMenu\');">
				</form>
			';
			break;
	}
...
function PagMenu()
{
	...
	if ($_SESSION['USR_ACE'] > 1) {
		$html_usr .= '<li><a id="novo_usuario" href="' . $_SERVER["PHP_SELF"] . '?pag=pagNovoUsuario" title="Novo Usuario">Usuarios</a></li>';
	}

	if ($_SESSION['USR_COD']) {
		$html .= '
			<li><a href="javascript:AjaxLoad(\'theme_conteudo_itens\',\'' . $_SERVER["PHP_SELF"] . '?pag=pagLista\');">Principal</a></li>	
			<li><a href="javascript:void(0);">Cadastro</a>
				<ul>
					' . $html_usr . '
					<li><a id="novo_grupo" href="' . $_SERVER["PHP_SELF"] . '?pag=pagNovoGrupo" title="Novo Grupo">Grupos</a></li>
					<li><a id="novo_contato" href="' . $_SERVER["PHP_SELF"] . '?pag=pagNovoContato" title="Novo Contato">Contatos</a></li>
				</ul>
			</li>
		';
	} else {
		$html .= '<li><a href="javascript:void(0);">Principal</a></li>';
	}
...
function PagLista()
{
	global $bd_conexao, $VAR_PES, $VAR_PES_GRU, $VAR_PES_UF;

	$contatos = "";

	print "<script>$('#theme_bar_login').load('" . $_SERVER["PHP_SELF"] . "?pag=loginMenu&cod=1');</script>";

	if ($VAR_PES_GRU) {
		$grupo = "AND GND_CTO_GRU = '" . $VAR_PES_GRU . "'";
	}
	if ($VAR_PES_UF) {
		$estado = "AND GND_CTO_EST = '" . $VAR_PES_UF . "'";
	}
	...
	if ($QueryResults > 0) {
		...
		while ($ResultRow = mysqli_fetch_array($QueryApply)) {
			...
			<tr id=\"lista_item$BDSHOW[1]\" style=\"background:$bgcolor\" class=\"lista_item\">
				<td>
				<a href=\"" . $_SERVER["PHP_SELF"] . "?pag=pagNovoContato&concod=$BDSHOW[1]&connome=$BDSHOW[2]&consnome=$BDSHOW[3]\" 
				id=\"Cont_Edit$BDSHOW[1]\" title=\"<div id='info_all'>Editar Contato</div>\"><img src=\"images/icons/6059_16x16.png\"></a>
				</td>
				</td>
				<td>
				<a href=\"" . $_SERVER["PHP_SELF"] . "?pag=alert&concod=$BDSHOW[1]&connome=$BDSHOW[2]&consnome=$BDSHOW[3]\" 
...
function PagNovoUsuario()
{
	...
	if ($FORM_USRCOD) {
		...
		if ($QueryEditResults > 0) {
			while ($ResultRow = mysqli_fetch_array($QueryEditApply)) {
				$BDEDIT[1]	= $ResultRow["WDK_USR_COD"];
				$BDEDIT[2] 	= $ResultRow["WDK_USR_NOM"];
				$BDEDIT[3] 	= $ResultRow["WDK_USR_USR"];
				$BDEDIT[4] 	= $ResultRow["WDK_USR_SNH"];
				$BDEDIT[5] 	= $ResultRow["WDK_USR_MAI"];
				$BDEDIT[6] 	= $ResultRow["WDK_USR_ACE"];
				$BDEDIT[7] 	= $ResultRow["WDK_USR_AUS"];
	...
	if ($QueryEditResults > 0) {
		while ($ResultRow = mysqli_fetch_array($QueryEditApply)) {
			...
			$usr_edit .= "<option value=" . $BDSHOW[1] . ">" . $BDSHOW[2] . "</option>";
		}
	}
	...
	if ($FORM_USRCOD) {
		...
		$QUERY_EWHE = "WHERE WDK_USR_COD <> " . $QUSR_COD;
		...
		$BUTCANCEL 	= "<input type=\"button\" value=\"Cancelar Edição\" onclick=\"AjaxLoad('fancybox-content','" . $_SERVER["PHP_SELF"] . "?pag=pagNovoUsuario');\">";
	} else {
	...
	if ($QueryResults > 0) {
		...
		while ($ResultRow = mysqli_fetch_array($QueryApply)) {
			...
			$usuarios .= "<option value=" . $BDSHOW[1] . " " . $SelGRU[$BDSHOW[1]] . ">" . $BDSHOW[2] . "</option>";
		}
	} else {
	...
	<script type="text/javascript">
		...
		{
			AjaxLoad("fancybox-content","' . $_SERVER["PHP_SELF"] . '?pag=pagNovoUsuario&cod="+varNome);
		}
	</script>
	...
	<div id="form_exdiv">
		...
		<br /><br />
		<input type="button" value="Sim" onclick="AjaxLoad(\'fancybox-content\',\'' . $_SERVER["PHP_SELF"] . '?pag=delUsuario&cod=' . $FORM_USRCOD . '\');">
		<input type="button" value="Não" onclick="$(\'#form_exdiv\').fadeOut(300);">
	</div>
	
	<form id="form_usuarios" action="' . $_SERVER["PHP_SELF"] . '" method="GET">
	<input type="hidden" name="pag" value="actUsuario">
	...
	<table>
		...
		<tr>
			<td>Login:</td>
			<td><input type="text" name="form_login" id="form_login" value="' . $BDEDIT[3] . '"></td>
		</tr>
		<tr>
			<td>Senha:</td>
			<td><input type="password" name="form_senha" id="form_senha" value="' . $BDEDIT[4] . '"></td>
		</tr>
		<tr>
...
function PagNovoGrupo()
{
	...
	<div id="form_cadastros">
	<form id="form_grupos" action="' . $_SERVER["PHP_SELF"] . '" method="GET">
	<input type="hidden" name="pag" value="actGrupo">
	<table>
		<tr>
			<td>Titulo:</td>
...
function PagNovoContato()
{
	...
	if ($FORM_CONCOD) {
		...
		if ($QueryEditResults > 0) {
			while ($ResultEditRow = mysqli_fetch_array($QueryEditApply)) {
	...
	<div id="form_cadastros">
	<form id="form_contatos" action="' . $_SERVER["PHP_SELF"] . '" method="GET">
	<input type="hidden" name="pag" value="actContato">
	' . $FORMEDIT . '
	<table>
		<tr>
			<td>Nome:</td>
